...
FileSys.ensureAndCreate = function(jsonPath, callback) {
  if (!fs.pathExistsSync(jsonPath)) {
    callback(
      'Filesys : json path : ' + path.resolve(jsonPath) + ' is not valid\n'
    );
    return false;
  }
  ...
  if (!fs.pathExistsSync(aliceAPI)) {
    callback(
      'Filesys : aliceAPI : ' + path.resolve(aliceAPI) + ' is not valid\n'
    );
    return false;
  }
  if (!fs.pathExistsSync(pixi_sound_map_src)) {
    callback(
      'Filesys : pixi_sound_map_src : ' +
        path.resolve(pixi_sound_map_src) +
        ' is not valid\n'
    );
    return false;
  }
