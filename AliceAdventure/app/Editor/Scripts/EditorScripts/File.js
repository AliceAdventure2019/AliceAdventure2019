    ...
    ELECTRON.dialog.showSaveDialog(
      {
        ...
        buttonLabel: 'Save',
        filters: [
          { name: 'AliceAdventureProject', extensions: [File.extension] }
        ]
      },
      _path => {
        // callback
...
File.SaveToPath = function(_path) {
  ...
  File.tempJsonObj.projectData.viewWidth =
    GameProperties.instance.projectData.viewWidth;
  File.tempJsonObj.projectData.viewHeight =
    GameProperties.instance.projectData.viewHeight;

  // Write JSON file
  FS.writeJsonSync(File.instance.path, File.tempJsonObj, {
    spaces: '\t',
    EOL: '\n'
  });
...
File.OpenFromPath = function(_path) {
  ...
  File.instance.gameProperties.settings.inventoryGridNum =
    data.settings.inventoryGridNum;
  ...
  // Init operation
  GameProperties.GetSceneById(
    GameProperties.instance.settings.startScene
  ).SelectOn();
